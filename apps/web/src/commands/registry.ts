import { createCommandDispatcher, type CommandDefinition, type CommandDispatcher, type KeybindingDefinition as GenericKeybindingDefinition } from "@zodiac/server";
...
export type KeybindingDefinition = GenericKeybindingDefinition<CommandContext>;
...
/** Priority order used only when a caller doesn't name its own active contexts -- Zodiac's own most-specific-first vocabulary. `@zodiac/server`'s dispatcher itself carries no opinion about what a "context" means for a given host. */
