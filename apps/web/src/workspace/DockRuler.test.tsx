...
describe("DockRuler", () => {
	...
	it("shades from the left edge up to the guide's ratio when docking left", () => {
		...
		expect(shade).toHaveStyle({ transform: "translate(0px, 0px)", width: "100px" }); // 1/4 of 400
	});
	...
	it("shades from the guide's ratio to the right edge when docking right", () => {
		...
		const shade = screen.getByTestId("dock-ruler-shade");
		expect(shade).toHaveStyle({ transform: "translate(300px, 0px)", width: "100px" }); // from 3/4 of 400 to the edge
	});
	...
	it("shades from the top edge down to the guide's ratio when docking top", () => {
		...
		const shade = screen.getByTestId("dock-ruler-shade");
		expect(shade).toHaveStyle({ transform: "translate(0px, 0px)", height: "50px" }); // 1/4 of 200
	});
	...
	it("shades from the guide's ratio to the bottom edge when docking bottom", () => {
		...
		const shade = screen.getByTestId("dock-ruler-shade");
		expect(shade).toHaveStyle({ transform: "translate(0px, 150px)", height: "50px" }); // from 3/4 of 200 to the edge
	});
