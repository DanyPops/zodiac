import { TerminalSquare } from "lucide";
...
export function renderTerminalTile(container: HTMLElement): void {
	...
	container.innerHTML = `
		<div class="h-full flex flex-col bg-gray-950">
			<div class="flex items-center gap-2 px-3 py-2 border-b border-gray-800 text-xs text-gray-400">
				<span class="terminal-header-icon"></span>
				<span>Agent Terminal \u00b7 synthetic session</span>
			</div>
	...
	`;
	container.querySelector(".terminal-header-icon")?.appendChild(icon(TerminalSquare, { size: 14 }));
}
