...
// OS process, proving zodiacd's own real vehicle-loopback execution
// strategy end-to-end (see daily-drive-vehicle-loopback-contribution.system.test.ts).
// Mirrors packages/server/src/contribution/fixtures/vehicle-loopback-fixture.mjs
// -- kept as its own copy since apps/service must stay independently
// testable without depending on a sibling package's own test fixtures
// (same precedent as packages/pi/test/fixtures/fake-pi-rpc.mjs).
// Controlled entirely by environment variables so the same file covers
// the happy path and both required failure modes (a crash after
// activation, and a hang that never responds) without needing three
// near-duplicate fixture files.
//
