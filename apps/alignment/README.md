...
## Window Carousel: centered, fading with distance, and an infinite loop

The active Window's own button always sits horizontally centered in the Carousel's track (a coverflow effect). Each Window is positioned individually via `circularWindowDelta(index, activeIndex, windowCount)` in `workspace/window-carousel-fade.ts` -- the *shortest wrapped* distance from the active Window, not a raw index difference, so the Window right before index 0 is the last one (delta -1), not "maximally far away". `computeWindowOffsetPx(delta)` turns that into a signed pixel offset (`left: 50%; transform: translateX(calc(-50% + offsetPx))` per button), and `computeWindowFadeOpacity(delta)` fades it out -- a linear falloff from full opacity at the active Window to fully invisible (`opacity: 0`, not just dim) at `WINDOW_FADE_DISTANCE` (3) Windows away or further. All three are plain, DOM-free functions, unit-tested directly.
...
**The mouse wheel is now the exact same wrap-around ring as clicks and the keyboard commands** (`window.next`/`window.previous`) -- `scrollWindow` in `workspace/model.ts` just delegates to `nextWindow`/`previousWindow`. It used to be a deliberately different policy: scrolling past either end spawned a new empty ("ephemeral") Window and pruned whichever one was left behind. That was reversed after live feedback ("the carousel should be infinity looping") and a real bug it had: the prune step swept *every* empty, inactive Window in one pass, not just the one just left, so a single wheel notch could collapse nearly the entire mock demo Carousel at once -- reported live as "mouse scrolling doesn't work". Scrolling is now pure navigation like the other two input methods: nothing is ever created or pruned by it. Explicit Window creation still exists via the New Window button/`window.new` (`Mod+Alt+N`), just no longer implicitly tied to scrolling past an edge.
