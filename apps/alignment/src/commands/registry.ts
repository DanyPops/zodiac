import { createCommandDispatcher, type CommandDefinition, type CommandDispatcher, type KeybindingDefinition as CoreKeybindingDefinition } from "@alignment/core";
...
/** Priority order used only when a caller doesn't name its own active contexts -- Alignment's own most-specific-first vocabulary. `@alignment/core`'s dispatcher itself carries no opinion about what a "context" means for a given host. */
