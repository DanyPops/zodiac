 ...
 * (last). Wraps at both ends -- see workspace/model.ts's nextWindow/
 * previousWindow, which this only drives via commands, never reimplements.
 */
export function WindowCarousel({ windowCount, activeIndex, onSelect }: WindowCarouselProps): React.JSX.Element {
	function handleWheel(event: WheelEvent<HTMLDivElement>): void {
		...
		onSelect(((activeIndex + (delta > 0 ? 1 : -1)) % windowCount + windowCount) % windowCount);
	}

	return (
		<nav aria-label="Window Carousel" className="flex h-10 shrink-0 items-center gap-1 border-b border-gray-200 bg-gray-50 px-2 dark:border-gray-700 dark:bg-gray-900" onWheel={handleWheel}>
