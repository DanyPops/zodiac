...
describe("HTTP conversation client", () => {
	it("parses the public summary without requiring a filesystem path", async () => {
		...
		const conversations = await createHttpConversationClient(fetcher as typeof fetch).list();
	...
	it("rejects malformed transport data", async () => {
		...
		await expect(createHttpConversationClient(fetcher as typeof fetch).list()).rejects.toThrow(/invalid-conversation/);
	});

	it("encodes the opaque conversation id when loading events", async () => {
		const fetcher = vi.fn(async () => jsonResponse({ events: [] }));
		await createHttpConversationClient(fetcher as typeof fetch).loadEvents("name/with spaces");
		expect(fetcher).toHaveBeenCalledWith("/api/events?conversationId=name%2Fwith%20spaces", expect.any(Object));
	});
});
