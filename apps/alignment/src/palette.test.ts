...
/**
 * Regression guard for a real, live bug: the shell's dark mode looked
 * blue-tinted, not a true black/grey scale. Root cause was Tailwind's own
 * "gray" palette (cool-toned, e.g. #111827 where blue clearly leads red and
 * green) plus dockview-core's "Abyss" dark theme (literally navy/purple at
 * its color roots -- #000c18, #1c1c2a, #2b2b4a). Reads styles.css directly
 * rather than duplicating hex values here, so this fails the moment either
 * palette regresses back to a tinted one, not just when someone remembers
 * to eyeball a screenshot.
 */
describe("neutral grayscale palette", () => {
