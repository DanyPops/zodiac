...
/**
 * Upper bound on lines scanned per file while looking for a session.name
 * event. Real data has shown a name arriving as late as line 208 of a
 * 221-line file (see readSessionName), so this must stay well above typical
 * session length -- it exists to cap a pathological/corrupt file, not to
 * second-guess where a real name normally appears.
 */
export const MAX_NAME_SCAN_LINES = 5_000;
...
export function readSessionName(filePath: string): Promise<string | undefined> {
	return new Promise((resolve) => {
		...
		rl.on("line", (line) => {
			linesScanned += 1;
			if (linesScanned > MAX_NAME_SCAN_LINES) {
				finish(undefined);
				return;
			}
			try {
		...
		// readline.Interface re-emits the underlying stream's own "error" as its
		// own event and consumes it -- without a listener here (not just on
		// `stream`), an unreadable file crashes the process instead of resolving.
