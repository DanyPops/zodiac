 ...
 * call) as a sequence of `data:` frames -- the one wire format every
 * zodiacd SSE route (World, agent sessions, and this same shape's own test
 * double in apps/service's daemon-multi-client.test.ts) actually produces:
 * `data: <json>\n\n`, nothing fancier (no multi-field events, no `id:`/
 * `retry:` lines). Deliberately hand-rolled rather than a dependency --
 * `Response.body` is a standard `ReadableStream<Uint8Array>` in both a
 * browser and modern Node (18+), so this one implementation works
 * unmodified in @zodiac/pi's terminal-side HTTP agent integration and this
 * package's own remote World store, with no platform-specific branch.
 *
