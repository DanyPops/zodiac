...
// personal Pi directory on every test run. The fixture commands here are
// plain `node` scripts, so PI_CODING_AGENT_DIR is inert either way -- these
// dirs exist purely to keep the seeding side effect hermetic.
let agentDirRoot: string | undefined;

function isolatedAgentDirs(): { agentDir: string; sourceAgentDir: string } {
	agentDirRoot = mkdtempSync(join(tmpdir(), "alignment-subprocess-integration-"));
